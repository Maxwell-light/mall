...
public class OrderServiceImpl implements IOrderService {
    ...
    public ResponseVo<OrderVo> create(Integer uid, Integer shippingId) {
        ...
        //通过uid获取购物车，校验（购物车中是否有商品，库存是否充足）
        ...
        //TODO 之前CartServiceImpl中，需要优化，使用mysql里的in进行查询
        ...
        //计算总价，只计算被选中的商品
        //生成订单，入库：order表和order_item表，事务
        Order order = buildOrder(uid, orderNo, shippingId, orderItemList);

        int rowForOrder = orderMapper.insertSelective(order);
        if (rowForOrder <= 0) {//影响的行数小于等于0，表示失败
            return ResponseVo.error(ResponseEnum.ERROR);
        }

        int rowForOrderItem = orderItemMapper.batchInsert(orderItemList);
        if (rowForOrderItem <= 0) {//影响的行数小于等于0，表示失败
            return ResponseVo.error(ResponseEnum.ERROR);
        }

        //更新购物车（删除选中的商品）
        //操作Redis，Redis有事务(打包命令)，不能回滚
        for (Cart cart : cartList) {
    ...
    public ResponseVo<PageInfo> list(Integer uid, Integer pageNum, Integer pageSize) {
        ...
        Map<Long, List<OrderItem>> orderItemMap = orderItemList.stream()
                .collect(Collectors.groupingBy(OrderItem::getOrderNo));//List转map
